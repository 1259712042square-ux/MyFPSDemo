...
namespace
{
FString RemoveToolNoiseFromChatTranscript(const FString& Transcript)
{
	...
	{
		const int32 ToolMarkerIndex = Line.Find(TEXT("[tool]:"), ESearchCase::IgnoreCase);
		if (ToolMarkerIndex != INDEX_NONE)
		{
			Line = Line.Left(ToolMarkerIndex).TrimEnd();
		}
		...
		else if (CleanLines.Num() > 0 && !CleanLines.Last().TrimStartAndEnd().IsEmpty())
		{
			CleanLines.Add(TEXT(""));
		}
	}

	FString CleanTranscript = FString::Join(CleanLines, TEXT("\n")).TrimStartAndEnd();
	while (CleanTranscript.Contains(TEXT("\n\n\n")))
	{
		CleanTranscript.ReplaceInline(TEXT("\n\n\n"), TEXT("\n\n"));
	}
...
void SGitHubCopilotUEPanel::Construct(const FArguments& InArgs)
{
	...
	if (BridgeService.IsValid())
	{
		...
		{
			ChatTranscriptBuffer = RemoveToolNoiseFromChatTranscript(SavedTranscript);
			if (ChatTranscriptBuffer != SavedTranscript)
			{
	...
	// --- Initialize dark mode text box styles ---
	{
		...
		DarkTextBoxStyle.TextStyle.SetFont(FSlateFontInfo(TEXT("C:\\UE_5.5\\Engine\\Content\\Slate\\Fonts\\DroidSansFallback.ttf"), 12));
		...
		[
			...
			[
				...
				[
					...
					.AutoWrapText(true)
					.OnKeyDownHandler(this, &SGitHubCopilotUEPanel::OnResponseTextKeyDown)
...
void SGitHubCopilotUEPanel::OnToolActivity(const FString& ActivityMessage)
{
	...
	{
		AppendToLog(FString::Printf(TEXT("Tool: %s"), *ActivityMessage));
	});
}
...
void SGitHubCopilotUEPanel::AppendChatTurn(const FString& Speaker, const FString& Message)
{
	...
	if (SafeSpeaker.Equals(TEXT("[tool]"), ESearchCase::IgnoreCase))
	{
		AppendToLog(FString::Printf(TEXT("Tool: %s"), *Message));
		return;
	}
	...
	if (!ChatTranscriptBuffer.IsEmpty())
	{
		...
		ChatTranscriptBuffer += Separator;
		ChatTranscriptBuffer += TEXT("\n\n");
	}

	ChatTranscriptBuffer += FString::Printf(TEXT("  [%s]  %s\n\n"), *Timestamp, *SafeSpeaker);
	ChatTranscriptBuffer += FString::Printf(TEXT("  %s\n"), *SafeMessage.Replace(TEXT("\n"), TEXT("\n  ")));
...
void SGitHubCopilotUEPanel::ScrollChatToEnd()
{
	if (ResponseTextBox.IsValid())
	{
		ResponseTextBox->GoTo(ETextLocation::EndOfDocument);
		ResponseTextBox->ScrollTo(ETextLocation::EndOfDocument);
	}
...
FReply SGitHubCopilotUEPanel::OnResponseTextKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply SGitHubCopilotUEPanel::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
	if (InKeyEvent.GetKey() == EKeys::End)
	{
		ScrollChatToEnd();
