...
void FGitHubCopilotUEModule::RegisterMenus()
{
	...
	// Add to the main Tools menu (Level Editor)
	{
	...
	// Add toolbar button (Level Editor)
	{
	...
	// Add toolbar button to Asset Editors (Blueprint, Animation BP, Widget BP, etc.)
	{
		UToolMenu* AssetToolbar = UToolMenus::Get()->ExtendMenu("AssetEditor.BlueprintEditor.ToolBar");
		FToolMenuSection& Section = AssetToolbar->FindOrAddSection("GitHubCopilot");
		FToolMenuEntry& Entry = Section.AddEntry(FToolMenuEntry::InitToolBarButton(FGitHubCopilotUECommands::Get().OpenPluginWindow));
		Entry.SetCommandList(PluginCommands);
	}
	// Add to asset editor Window menu as fallback
	{
		UToolMenu* WindowMenu = UToolMenus::Get()->ExtendMenu("AssetEditor.MainMenu.Window");
		FToolMenuSection& Section = WindowMenu->FindOrAddSection("GitHubCopilot");
		Section.AddMenuEntryWithCommandList(
			FGitHubCopilotUECommands::Get().OpenPluginWindow,
			PluginCommands);
	}
