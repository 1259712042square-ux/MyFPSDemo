...
struct FCopilotMessageSanitizeStats
{
	int32 RemovedUnsupportedContentParts = 0;
	int32 RemovedOrphanToolMessages = 0;
	int32 InsertedMissingToolResults = 0;
	int32 RemovedNestedToolBlocks = 0;

	bool HasChanges() const
	{
		return RemovedUnsupportedContentParts > 0
			|| RemovedOrphanToolMessages > 0
			|| InsertedMissingToolResults > 0
			|| RemovedNestedToolBlocks > 0;
	}
};

FString GetMessageRole(const TSharedPtr<FJsonObject>& Message)
{
	FString Role;
	if (Message.IsValid())
	{
...
bool MessageHasToolCalls(const TSharedPtr<FJsonObject>& Message)
{
	const TArray<TSharedPtr<FJsonValue>>* ToolCalls = nullptr;
	return Message.IsValid()
		&& Message->TryGetArrayField(TEXT("tool_calls"), ToolCalls)
		&& ToolCalls
		&& ToolCalls->Num() > 0;
}

TArray<FString> GetToolCallIds(const TSharedPtr<FJsonObject>& Message)
{
	TArray<FString> Ids;
	const TArray<TSharedPtr<FJsonValue>>* ToolCalls = nullptr;
	if (!Message.IsValid() || !Message->TryGetArrayField(TEXT("tool_calls"), ToolCalls) || !ToolCalls)
	{
		return Ids;
	}
	...
	for (const TSharedPtr<FJsonValue>& ToolCallValue : *ToolCalls)
	{
		...
		FString Id;
		if (ToolCall->TryGetStringField(TEXT("id"), Id) && !Id.IsEmpty())
		{
			Ids.Add(Id);
		}
	}
...
TSharedPtr<FJsonValue> MakeMissingToolResult(const FString& ToolCallId)
{
	TSharedPtr<FJsonObject> DummyResult = MakeShareable(new FJsonObject);
	DummyResult->SetStringField(TEXT("role"), TEXT("tool"));
	DummyResult->SetStringField(TEXT("tool_call_id"), ToolCallId);
	DummyResult->SetStringField(TEXT("content"), TEXT("[result unavailable]"));
	return MakeShareable(new FJsonValueObject(DummyResult));
}

bool IsClaudeToolContentBlock(const TSharedPtr<FJsonObject>& Object)
{
	if (!Object.IsValid())
	{
		return false;
	...
	return Type == TEXT("tool_result")
		|| Type == TEXT("tool_use")
		|| Object->HasField(TEXT("tool_use_id"))
		|| Object->HasField(TEXT("tool_result"))
		|| Object->HasField(TEXT("tool_use"));
}

void SanitizeJsonObjectForCopilotRequest(const TSharedPtr<FJsonObject>& Object, FCopilotMessageSanitizeStats& Stats);

void SanitizeJsonArrayForCopilotRequest(TArray<TSharedPtr<FJsonValue>>& Array, FCopilotMessageSanitizeStats& Stats)
{
	for (int32 Index = 0; Index < Array.Num(); )
	{
		...
		if (IsClaudeToolContentBlock(Object))
		{
			Array.RemoveAt(Index);
			++Stats.RemovedNestedToolBlocks;
			continue;
		}

		if (Object.IsValid())
		{
			SanitizeJsonObjectForCopilotRequest(Object, Stats);
		}
		else if (Value.IsValid() && Value->Type == EJson::Array)
		{
			TArray<TSharedPtr<FJsonValue>> NestedArray = Value->AsArray();
			SanitizeJsonArrayForCopilotRequest(NestedArray, Stats);
			Array[Index] = MakeShareable(new FJsonValueArray(NestedArray));
		}
		++Index;
	}
}
...
void SanitizeJsonObjectForCopilotRequest(const TSharedPtr<FJsonObject>& Object, FCopilotMessageSanitizeStats& Stats)
{
	...
	for (TPair<FString, TSharedPtr<FJsonValue>>& Field : Object->Values)
	{
		...
		if (Value->Type == EJson::Object)
		{
			...
			if (IsClaudeToolContentBlock(ChildObject))
			{
				Field.Value = MakeShareable(new FJsonValueString(TEXT("[tool content block omitted]")));
				++Stats.RemovedNestedToolBlocks;
			}
			else
			{
				SanitizeJsonObjectForCopilotRequest(ChildObject, Stats);
			}
		}
		else if (Value->Type == EJson::Array)
		{
			TArray<TSharedPtr<FJsonValue>> Array = Value->AsArray();
			SanitizeJsonArrayForCopilotRequest(Array, Stats);
			if (Field.Key == TEXT("content") && Array.Num() == 0)
			{
				Field.Value = MakeShareable(new FJsonValueString(TEXT("[unsupported content omitted]")));
			}
			else
			{
				Field.Value = MakeShareable(new FJsonValueArray(Array));
			}
		}
	}
}

void SanitizeMessageContent(const TSharedPtr<FJsonObject>& Message, FCopilotMessageSanitizeStats& Stats)
{
	if (!Message.IsValid() || !Message->HasTypedField<EJson::Array>(TEXT("content")))
	{
		return;
	}

	const FString Role = GetMessageRole(Message);
	const TArray<TSharedPtr<FJsonValue>> ContentParts = Message->GetArrayField(TEXT("content"));
	...
	if (Role == TEXT("tool"))
	{
		...
		{
			const TSharedPtr<FJsonObject> Part = PartValue.IsValid() ? PartValue->AsObject() : nullptr;
			if (!Part.IsValid())
			{
				continue;
			}
			...
			FString Text;
			if (Part->TryGetStringField(TEXT("text"), Text) || Part->TryGetStringField(TEXT("content"), Text))
			{
				if (!ToolText.IsEmpty())
				{
					ToolText += TEXT("\n");
		...
		Message->SetStringField(TEXT("content"), ToolText.IsEmpty() ? TEXT("[tool output omitted]") : ToolText);
		Stats.RemovedUnsupportedContentParts += ContentParts.Num();
		return;
	}

	TArray<TSharedPtr<FJsonValue>> SanitizedParts;
	for (const TSharedPtr<FJsonValue>& PartValue : ContentParts)
	{
		const TSharedPtr<FJsonObject> Part = PartValue.IsValid() ? PartValue->AsObject() : nullptr;
		if (!Part.IsValid())
		{
			++Stats.RemovedUnsupportedContentParts;
			continue;
		}

		FString Type;
		Part->TryGetStringField(TEXT("type"), Type);

		if (IsClaudeToolContentBlock(Part))
		{
			++Stats.RemovedUnsupportedContentParts;
			continue;
		}

		if (Type == TEXT("text") || Type == TEXT("image_url"))
		{
			SanitizedParts.Add(PartValue);
			continue;
		}
		...
		if (Part->TryGetStringField(TEXT("text"), Text) && !Text.IsEmpty())
		{
			...
			SanitizedParts.Add(MakeShareable(new FJsonValueObject(TextPart)));
			++Stats.RemovedUnsupportedContentParts;
			continue;
		}
	...
	if (SanitizedParts.Num() > 0)
	{
		Message->SetArrayField(TEXT("content"), SanitizedParts);
	}
	else
	{
		Message->SetStringField(TEXT("content"), TEXT("[unsupported content omitted]"));
	}
}

FCopilotMessageSanitizeStats NormalizeCopilotMessageSequence(TArray<TSharedPtr<FJsonValue>>& Messages)
{
	FCopilotMessageSanitizeStats Stats;
	TArray<TSharedPtr<FJsonValue>> Normalized;

	for (int32 Index = 0; Index < Messages.Num(); ++Index)
	{
		TSharedPtr<FJsonObject> Message = Messages[Index].IsValid() ? Messages[Index]->AsObject() : nullptr;
		if (!Message.IsValid())
		{
			continue;
		}

		SanitizeMessageContent(Message, Stats);
		const FString Role = GetMessageRole(Message);

		if (Role == TEXT("tool"))
		{
			++Stats.RemovedOrphanToolMessages;
			continue;
		}

		if (!MessageHasToolCalls(Message))
		{
			Normalized.Add(Messages[Index]);
			continue;
		}

		const TArray<FString> NeededIds = GetToolCallIds(Message);
		TSet<FString> NeededIdSet;
		...
		TMap<FString, TSharedPtr<FJsonValue>> ResultsById;
		TArray<TSharedPtr<FJsonValue>> DeferredMessages;

		int32 ScanIndex = Index + 1;
		for (; ScanIndex < Messages.Num(); ++ScanIndex)
		{
			TSharedPtr<FJsonObject> NextMessage = Messages[ScanIndex].IsValid() ? Messages[ScanIndex]->AsObject() : nullptr;
			if (!NextMessage.IsValid())
			{
				continue;
			}

			SanitizeMessageContent(NextMessage, Stats);
			const FString NextRole = GetMessageRole(NextMessage);
			if (NextRole == TEXT("assistant") && MessageHasToolCalls(NextMessage))
			{
				break;
			}
			...
			{
				FString ToolCallId;
				if (NextMessage->TryGetStringField(TEXT("tool_call_id"), ToolCallId)
					&& NeededIdSet.Contains(ToolCallId)
					&& !ResultsById.Contains(ToolCallId))
				{
					ResultsById.Add(ToolCallId, Messages[ScanIndex]);
				}
				else
				{
					++Stats.RemovedOrphanToolMessages;
			...
			DeferredMessages.Add(Messages[ScanIndex]);
		}

		for (const FString& NeededId : NeededIds)
		{
			if (TSharedPtr<FJsonValue>* ExistingResult = ResultsById.Find(NeededId))
			{
				Normalized.Add(*ExistingResult);
			}
			else
			{
				Normalized.Add(MakeMissingToolResult(NeededId));
				++Stats.InsertedMissingToolResults;
			}
		}

		Normalized.Append(DeferredMessages);
		Index = ScanIndex - 1;
	}

	Messages = MoveTemp(Normalized);
	return Stats;
}
} // namespace
...
void FGitHubCopilotUEBridgeService::Initialize()
{
	...
	if (IsAuthenticated())
	{
		Log(TEXT("BridgeService: Already authenticated via custom token"));
...
void FGitHubCopilotUEBridgeService::LoadTokenCache()
{
	...
	if (FJsonSerializer::Deserialize(Reader, Json) && Json.IsValid())
	{
		...
		// Custom API key support (DeepSeek, OpenAI, etc.)
		if (Json->HasField(TEXT("custom_token")))
		{
			CopilotToken = Json->GetStringField(TEXT("custom_token"));
			CopilotTokenExpiry = FDateTime::UtcNow().ToUnixTimestamp() + 999999;
			CopilotTokenIssuedAt = FPlatformTime::Seconds();
			CopilotTokenRefreshIn = 999999;
			SetAuthState(ECopilotAuthState::Authenticated);
			SetConnectionStatus(ECopilotConnectionStatus::Connected);
			Log(FString::Printf(TEXT("BridgeService: Using custom API token, endpoint=%s, model=%s"),
				*CopilotAPIBase, *ActiveModelId));
		}
...
void FGitHubCopilotUEBridgeService::SendChatCompletion(const FCopilotRequest& Request, bool bAllowToolCalls)
{
	...
	// the user's original message first. This rewrite protects all user messages.
	{
		...
		FCopilotMessageSanitizeStats SanitizeStats = NormalizeCopilotMessageSequence(ConvoMessages);
		if (SanitizeStats.HasChanges())
		{
			Log(FString::Printf(
				TEXT("BridgeService: [VALIDATE] Sanitized conversation: removed_content_parts=%d, removed_orphan_tools=%d, inserted_missing_tool_results=%d, removed_nested_tool_blocks=%d"),
				SanitizeStats.RemovedUnsupportedContentParts,
				SanitizeStats.RemovedOrphanToolMessages,
				SanitizeStats.InsertedMissingToolResults,
				SanitizeStats.RemovedNestedToolBlocks));
		}
	}
...
// Strip image_url parts for non-vision endpoints (e.g. DeepSeek)
		if (CopilotAPIBase.Contains(TEXT("deepseek")))
		{
			for (const TSharedPtr<FJsonValue>& MsgVal : ConvoMessages)
			{
				TSharedPtr<FJsonObject> Msg = MsgVal.IsValid() ? MsgVal->AsObject() : nullptr;
				if (!Msg.IsValid() || !Msg->HasTypedField<EJson::Array>(TEXT("content"))) continue;
				TArray<TSharedPtr<FJsonValue>> Parts = Msg->GetArrayField(TEXT("content"));
				TArray<TSharedPtr<FJsonValue>> TextOnly;
				for (const auto& Part : Parts)
				{
				...
				if (TextOnly.Num() != Parts.Num())
				{
					...
					{
						Msg->SetStringField(TEXT("content"), TEXT("[image was here but removed for text-only model]"));
					}
				}
			}
		}
	...
	FCopilotMessageSanitizeStats RequestBodySanitizeStats;
	SanitizeJsonObjectForCopilotRequest(JsonBody, RequestBodySanitizeStats);
	if (RequestBodySanitizeStats.HasChanges())
	{
		Log(FString::Printf(
			TEXT("BridgeService: [VALIDATE] Sanitized final request body: removed_content_parts=%d, removed_orphan_tools=%d, inserted_missing_tool_results=%d, removed_nested_tool_blocks=%d"),
			RequestBodySanitizeStats.RemovedUnsupportedContentParts,
			RequestBodySanitizeStats.RemovedOrphanToolMessages,
			RequestBodySanitizeStats.InsertedMissingToolResults,
			RequestBodySanitizeStats.RemovedNestedToolBlocks));
	}
...
void FGitHubCopilotUEBridgeService::OnChatCompletionResponse(FHttpRequestPtr HttpReq, FHttpResponsePtr HttpResp, bool bSuccess, FString RequestId, FString ConversationId)
{
	...
	if (!bSuccess || !HttpResp.IsValid())
	{
		...
		ToolHistoryRepairRetryRequestIds.Remove(RequestId);
	...
	{
		const bool bMalformedToolHistory400 = StatusCode == 400
			&& (Body.Contains(TEXT("tool_use_id")) || Body.Contains(TEXT("tool_result")) || Body.Contains(TEXT("tool_use")));
		if (bMalformedToolHistory400 && !ToolHistoryRepairRetryRequestIds.Contains(RequestId))
		{
			ToolHistoryRepairRetryRequestIds.Add(RequestId);
			if (TArray<TSharedPtr<FJsonValue>>* ConvoMessages = ActiveConversations.Find(ConversationId))
			{
				FCopilotMessageSanitizeStats SanitizeStats = NormalizeCopilotMessageSequence(*ConvoMessages);
				Log(FString::Printf(
					TEXT("BridgeService: Repaired malformed tool history after HTTP 400: removed_content_parts=%d, removed_orphan_tools=%d, inserted_missing_tool_results=%d, removed_nested_tool_blocks=%d"),
					SanitizeStats.RemovedUnsupportedContentParts,
					SanitizeStats.RemovedOrphanToolMessages,
					SanitizeStats.InsertedMissingToolResults,
					SanitizeStats.RemovedNestedToolBlocks));
			...
			RetryRequest.CommandType = ECopilotCommandType::Ask;
			PendingRequestTimestamps.Add(RequestId, FPlatformTime::Seconds());
			SendChatCompletion(RetryRequest, !ForcedFinalResponseRequestIds.Contains(RequestId));
			return;
		}
	...
	if (FinishReason == TEXT("tool_calls") || Message->HasField(TEXT("tool_calls")))
	{
		...
		IterCount++;
		const UGitHubCopilotUESettings* Settings = UGitHubCopilotUESettings::Get();
		constexpr int32 SafeDefaultMaxToolLoopIterations = 12;
		const int32 ConfiguredMaxToolLoopIterations = Settings ? Settings->MaxToolCallIterations : SafeDefaultMaxToolLoopIterations;
		const int32 MaxToolLoopIterationsBeforeFinalization = ConfiguredMaxToolLoopIterations > 0
			? ConfiguredMaxToolLoopIterations
			: SafeDefaultMaxToolLoopIterations;
		if (IterCount > MaxToolLoopIterationsBeforeFinalization)
		{
...
void FGitHubCopilotUEBridgeService::SaveConversationCache()
{
	...
	TArray<TSharedPtr<FJsonValue>>* ConvoPtr = ActiveConversations.Find(CurrentConversationId);
	if (!ConvoPtr || ConvoPtr->Num() == 0)
	{
		return;
	}

	FCopilotMessageSanitizeStats SanitizeStats = NormalizeCopilotMessageSequence(*ConvoPtr);
	if (SanitizeStats.HasChanges())
	{
		Log(FString::Printf(
			TEXT("BridgeService: Sanitized conversation before saving: removed_content_parts=%d, removed_orphan_tools=%d, inserted_missing_tool_results=%d, removed_nested_tool_blocks=%d"),
			SanitizeStats.RemovedUnsupportedContentParts,
			SanitizeStats.RemovedOrphanToolMessages,
			SanitizeStats.InsertedMissingToolResults,
			SanitizeStats.RemovedNestedToolBlocks));
...
void FGitHubCopilotUEBridgeService::LoadConversationCache()
{
	...
	{
		Log(FString::Printf(
			TEXT("BridgeService: Sanitized restored conversation: removed_content_parts=%d, removed_orphan_tools=%d, inserted_missing_tool_results=%d, removed_nested_tool_blocks=%d"),
			SanitizeStats.RemovedUnsupportedContentParts,
			SanitizeStats.RemovedOrphanToolMessages,
			SanitizeStats.InsertedMissingToolResults,
			SanitizeStats.RemovedNestedToolBlocks));
