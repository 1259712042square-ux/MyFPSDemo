...
- **Tool history sanitizer fixed** — prevents Copilot API HTTP 400 failures from stale or malformed `tool_use` / `tool_result` history by stripping invalid Claude-style content blocks, removing orphaned tool messages, and inserting missing tool results before requests are sent or conversations are saved/restored
- **Conversation resume hardened** — saved chat state is normalized on load/save so one bad tool-call turn cannot keep poisoning later Unreal chat requests
- **Chat transcript cleaned up** — tool calls, tool results, and reasoning/progress spam now stay out of the main chat box; saved transcripts are cleaned on load and tool progress is routed to the execution log instead
- **Safe tool-loop default** — tool-call loops now use a safe default cap of 12 iterations so repeated project scans/web searches are forced into a final answer instead of running forever
...
- **Live tool activity in execution log** — see what the AI is doing without flooding the main chat transcript
...
| Max Tool-Call Iterations (0 = Safe Default) | `12` | Iteration cap for tool-call loops; `0` falls back to the safe default |
...
The loop continues until the AI returns a text response with no further tool calls. A configurable cap exists in settings (`Max Tool-Call Iterations`), and the default safe cap is `12` iterations (`0` falls back to that safe default).
